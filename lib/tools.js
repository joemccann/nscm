...
function getOptions (opts, callback) {
  ...
  if (!publicRegistry) {
    return callback(new Error('The publicRegistry option is required!'))
  }
  ...
  if (!registry) {
    getRegistry(function (err, r) {
      if (err) {
        return callback(new Error('A registry is required!'))
      }
  ...
  function setToken (opts, callback) {
    if (!token) {
      getJWT(opts.registry, function (err, t) {
        if (err) {
          return callback(new Error('A token is required. Please use `nscm signin`.'))
        }
...
function getPackages (options, callback) {
  ...
  function execute (err) {
    if (err) {
      console.error('No `package.json` file was found. Please run `nscm` inside a Node.js project.')
      return
    }
...
function runNpm (options, callback) {
  ...
  console.error('Please wait while we score your Node modules...')
  exec(cmd, {
...
function checkPackages (opts, callback) {
  ...
  mapLimit(packages, opts.concurrency || 15, function (payload, next) {
    ...
    request({
      url: `${opts.registry}/api/v1/package?name=${payload.name}&version=${payload.version}`,
      method: 'GET',
      headers: {
...
function parseJSON (str) {
  ...
  try {
    ...
    console.error('An error ocurred while parsing the output.', err.message)
  }
...
function usageFilter (usage) {
  ...
  Additional Help
    ...
    Add -h to the 'config' and 'whitelist' commands for additional help with those commands.
